...
Данный интерфейс позволяет подключать HTML-шаблоны, CSS, JavaScript и изображения.
...
* **static** — хранение CSS, JavaScript и изображений
...
```html
<link rel="stylesheet" href="{{ url_for('ИмяПлагина.static', filename='styles.css') }}">
<script src="{{ url_for('ИмяПлагина.static', filename='script.js') }}" defer></script>
```
...
* `render_template` имеет асинхронную версию и должен вызываться с `await`
...
- **`name`**: *str* - Название вашего плагина (должно быть уникальным).
...
- **`tags`**: *list[str]* - Ключевые слова или категории, по которым можно найти ваш плагин (напр. `"admin"`, `"fun"`, `"utility"`).
