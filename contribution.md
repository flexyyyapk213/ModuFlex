...
- **`timeout_download_lib`** – отвечает за ограничение по времени (в секундах) скачивания библиотеки для плагина. Следует указывать только целое корректное число, по умолчанию - 60 (секунд).

  - Рядом с числом нечего лишнего быть не должно.
  - Пример:
...
- **`check_for_update`** – Проверять ли обновления с интервалом.
  - `true` – Да(По умолчанию.С интервалом в 10 минут).
  - `false` – Нет.
